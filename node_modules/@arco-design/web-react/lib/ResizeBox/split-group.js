...
function SplitGroup(props, ref) {
    ...
    var _d = __read((0, react_1.useState)(new Array(panes.length).fill(defaultOffset)), 2), offsets = _d[0], setOffsets = _d[1];
    var _e = __read((0, react_1.useState)(false), 2), isMoving = _e[0], setIsMoving = _e[1];
    var _f = __read((0, react_1.useState)(new Array(panes.length).fill(0)), 2), triggerSize = _f[0], setTriggerSize = _f[1];
    var _g = __read((0, react_1.useState)(new Array(Math.max(panes.length - 1, 0)).fill({ prev: false, next: false })), 2), collapsedStatus = _g[0], setCollapsedStatus = _g[1];
    ...
    var classNames = (0, classNames_1.default)(prefixCls, prefixCls + "-" + (isHorizontal ? DIRECTION_HORIZONTAL : DIRECTION_VERTICAL), (_a = {}, _a[prefixCls + "-moving"] = isMoving, _a), className);
    var Tag = component;
    ...
    var getInitialOffsets = function () {
        ...
        var remainPx = totalPx -
            newOffsets.reduce(function (a, b) {
                var formatA = a || 0;
                var formatB = b || 0;
                return formatA + formatB;
            }, 0);
        var averagePx = remainPx / noSizeArr.length;
    ...
    var getNewOffsets = function (startOffset, startPosition, currentPosition) {
        ...
        var currentPx = offsets[current];
        var nextPx = offsets[next];
        var totalOffset = currentPx + nextPx;
        var _a = getMinAndMax(current), minOffset = _a.currentMin, maxOffset = _a.currentMax;
        var moveOffset = startOffset + (currentPosition - startPosition);
    ...
    function moving(e) {
        ...
        if (currentRecord.moving) {
            ...
            props.onMoving &&
                props.onMoving(e, newOffsets.map(function (value) { return value + "px"; }), index);
        }
    }
    // 移动结束，解除事件绑定
    function moveEnd() {
    ...
    function handleCollapsed(e, index, status, callback) {
        ...
        props.onMoving &&
            props.onMoving(e, newOffset.map(function (value) { return value + "px"; }), index);
        props.onMovingEnd && props.onMovingEnd(index);
    ...
    return (react_1.default.createElement(Tag, { style: style, className: classNames, ref: wrapperRef }, panes.map(function (pane, index) {
